...
def post_random(query,budget):
    ...
    if budget is None:
        ...
        random_ingridient_quality_dict = {}
        for ingredient in random_meal['ingredients']:
            random_quality = random.choice(['high','medium','low'])
            random_ingridient_quality_dict[ingredient['name'].lower()] = random_quality
        
        price = find_price_of_meal(random_meal['id'],random_ingridient_quality_dict)
        quality =find_quality_of_meal(random_meal['id'],random_ingridient_quality_dict)
        
        result_ingredients_list = []
        for ingredient in random_meal['ingredients']:
            result_ingredients_list.append({'name': ingredient['name'], 'quality': random_ingridient_quality_dict[ingredient['name'].lower()]})
    ...
    while True:
        ...
        unused_ingridient_combinations = {}

        random_meal = random.choice(meals_list)
        meals_list.remove(random_meal)

        for ingredient in random_meal['ingredients']:
            unused_ingridient_combinations[ingredient['name'].lower()] = ['high','medium','low']
        
        query_ingridient = {}
        for ingredient, unused_qualities in unused_ingridient_combinations.items():
            random_quality = random.choice(unused_qualities) # Choose a random quality from the unused qualities of the ingredient
            query_ingridient[ingredient.lower()] = random_quality # Add the quality to the query_ingridient_dict for sending to the find_price_of_meal function
            unused_qualities.remove(random_quality) # Remove the quality that is used so that combination algorithm does not use it again
        ...
        formatted_ingridient_list = []
        for ingredient, quality in query_ingridient.items():
            formatted_ingridient_list.append({'name': ingredient, 'quality': quality})
        ...
        if price < budget:
            return {
                    ...
                    'ingredients': formatted_ingridient_list
                    }, HTTPStatus.OK